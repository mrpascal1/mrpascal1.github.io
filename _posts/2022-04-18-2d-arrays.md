...
*   Reviewer at 0<sup>th</sup> index has given rating to the Movie at 0<sup>th</sup> index.
*   To access 4 in the 2D array we write list[row][column] and <b>replace row with 0</b> and <b>column with 0</b> like this - list[0][0].
...
To find length of 2D array, generally we consider the number of rows present in it:
```python
# To find length of an array will use len() method.
print(len(ratings)) # Output: 3
```

Iterate over elements of 2D array:<br>
```python
...
for row in range(len(ratings)):
    sum += ratings[row][2]
print(sum / len(ratings)) 
```

Similary, what if we want to increment each reviewer rating by 2 i.e each element in an array by 2:
```python
for row in range(len(ratings)):
...
<b>Interview problem -</b> How will you rotate a matrix 
